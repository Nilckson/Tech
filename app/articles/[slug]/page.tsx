        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "680px", margin: "0 auto", padding: "0 clamp(1rem,5vw,2rem)" }}>
          ...
          <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.5rem 0" }}>
            <Link href="/" style={{
              ...
              letterSpacing: "0.1em", color: "#00f2fe", textTransform: "uppercase", textDecoration: "none",
            }}>
              NilcksonTech
            </Link>
            <Link href="/" style={{
              fontSize: "0.8rem", color: "#2d4a5e", textDecoration: "none", fontFamily: "'DM Sans', sans-serif",
            }}>
              ← Back
            </Link>
          </nav>
          ...
          <article className="prose" style={{ paddingBottom: "5rem" }}>
            {lines.map((line, i) => {
              ...
              if (line.startsWith("- ")) return (
                <ul key={i}><li dangerouslySetInnerHTML={{ __html: line.replace("- ", "").replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>") }} /></ul>
              );
              if (line.match(/^\d\./)) return (
                <ol key={i}><li dangerouslySetInnerHTML={{ __html: line.replace(/^\d\.\s/, "").replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>") }} /></ol>
              );
